...
> Addendum 2026-09-05: the deployed `update_live_session_with_event_v2` RPC locks `status = 'preparing'`, so the push cannot reuse the host PATCH path. Task 6a adds migration `supabase/migrations/202609020004_live_session_engine_admin.sql` with `set_live_session_engine_admin_v1(p_actor_id uuid, p_session_id uuid, p_engine jsonb) returns table (id uuid, status text, version integer)` — `security definer`, `set search_path = ''`, requires the actor to be an approved admin (reuse `assert_console_admin_v1`), applies only to sessions with `status in ('preparing','live')` and `archive_deleted_at is null`, rewrites `event_metadata.modelPreferences.engine`, appends `{ engine, changedAt, byHostId: actor.host_id }` to `event_metadata.modelPreferences.engineHistory` (cap 64, oldest dropped), bumps `version`, and returns nothing for non-matching sessions; plus `list_live_session_ids_admin_v1() returns table (id uuid, status text, languages text[])` for the active set. Revoke/grant pattern as usual; mirrored into `bootstrap-new-project.sql`; PGlite test in `test/live-session-engine-admin-sql.test.js`. `console-store.ts` gains `listActiveSessions()` and `setSessionEngineAsAdmin({ actorId, sessionId, engine })`. Task 6b (after Plan 2 Task 5) wires the PUT route to call these and `pushEngineToGateway`.
