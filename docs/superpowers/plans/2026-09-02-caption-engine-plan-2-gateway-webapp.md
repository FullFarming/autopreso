...
### Task 5: Live Call hot swap and `engine-status`

**Files:**
- Modify: `electron/main.js` (settings-change hook: when `armedSession` exists and `engineSelectionKey` changed → `PATCH /api/live-sessions/:id { modelPreferences: { engine } }` then send `{ type: "update", ... gatewaySettings with new captionConfig }` reusing the restart sender), `webapp/components/live/LiveHostDashboard.tsx` + `live-audio-client.ts` (`update()` after PATCH when the host changes the engine mid-call), `media-gateway/src/gateway-server.js` (`engine-status` host event on pipeline start/ready/failed: `{ type: "engine-status", sessionId, role, provider, model, status, code? }`), `public/subtitle-controller.js` (show engine status pill), `webapp` host dashboard status line
- Tests: root `test/desktop-live-*.test.js` (vm-sliced hook), gateway `gateway-server.test.js` (update with changed fingerprint → new pipeline, old closed, seq continuity, viewers get `language-status preparing→ready`), webapp `live-audio-client.test.ts`

- [ ] **Step 1: Failing tests**; **Step 2: RED**; **Step 3: Implement**: desktop hook lives next to `saveSettingsAndApply`'s trigger — the server broadcasts `settings` over the local WS; `electron/main.js` already listens for settings changes for glossary; add the engine branch: `if (armedSession && engineSelectionKey(prev) !== engineSelectionKey(next)) await hotSwapLiveCallEngine(next)` with the PATCH → `update` ordering and a single in-flight guard. `engine-status` is sent from `gateway-server.js` where `language-status` is sent today (start: `connecting`; pipeline `start()` resolved: `ready`; `failOwnedPipeline`: `failed` with code). **Step 4: GREEN**; **Step 5: Commit** `feat(live-call): immediate engine hot swap for running sessions and engine-status events`
