...
export function buildPolishUserPrompt({ text, sourceText, targetLanguage, tone }) {
...
export function preparePolishRequest({
  ...
  if (tone !== "business" && !String(glossary ?? "").trim() && !domain && !shouldRecoverPlaceholder) return null;
  if (text.length < MIN_POLISH_CHARS && !shouldRecoverPlaceholder) return null;
  return {
    originalText,
    ...
    prompt: buildPolishUserPrompt({ text: text.slice(0, MAX_POLISH_CHARS), sourceText: source, targetLanguage: target, tone }),
  };
}
