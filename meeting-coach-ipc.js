 ...
 * boundsStore?: BoundsStoreLike, engine?: MeetingCoachEngineLike}} [options]
 */
export function registerMeetingCoachIpc({
  ...
  const captionSubscriber = createCanonicalCaptionSubscriber({
    ...
    onCommitted: (turn) => { void hydration.then(() => engine.acceptFinalizedTurn(turn)).catch(() => {}); },
  ...
  const handle = (channel, allowedKinds, operation) => {
    ...
    ipc.handle(channel, async (event, input) => {
      try {
        ...
        return { ok: false, error: userFacingError(error), code: normalizeErrorCode(error) };
      }
    });
  };
...
function userFacingError(error) {
  if (isRecord(error) && (error.code === "OPENAI_TIMEOUT" || error.code === "GEMINI_TIMEOUT")) {
    return "응답 시간이 초과되었습니다. 다시 시도해 주세요.";
  }
  return cleanText(error instanceof Error ? error.message : "", 300) || "Meeting Coach 요청을 처리하지 못했습니다.";
}
