...
import { glossaryPresetIdSchema, parseActivateDocumentBody } from "@/lib/glossary-presets/schema";
...
export async function POST(request: NextRequest, context: RouteContext) {
  try {
    ...
    const body = parseActivateDocumentBody(await readBoundedJsonBody(request));
    if (!parsedId.success || !body) return invalidInput();
    const activation = await getGlossaryPresetService().activateVersion(
      hostId, parsedId.data, body.presetVersion, body.documentVersion,
    );
    return apiSuccess({ activation }, { headers: privateNoStoreHeaders() });
