...
export async function POST(request: NextRequest) {
  try {
    ...
    const service = getGlossaryPresetService();
    const validateOnly = request.nextUrl.searchParams.get("validateOnly");
    if (validateOnly !== null && validateOnly !== "true") {
      return apiError("검증 옵션이 올바르지 않습니다.", "INVALID_GLOSSARY_PRESET", 400, privateNoStoreHeaders());
    }
    if (validateOnly === "true") {
      const validated = service.validate(document);
      return apiSuccess({ fingerprint: validated.fingerprint, document: validated.document }, { headers: privateNoStoreHeaders() });
    }
    const preset = await service.create(hostId, document);
