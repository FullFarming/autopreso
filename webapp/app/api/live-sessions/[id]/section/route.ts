...
import { sectionTransitionInputSchema } from "@/lib/security/live-input-validation";
...
export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const parsed = sectionTransitionInputSchema.safeParse(await readBoundedJsonBody(request));
    if (!parsed.success) return apiError("구간 전환 요청이 올바르지 않습니다.", "INVALID_REQUEST", 400);
    const session = await new LiveSessionService(getLiveSessionStore()).transitionSection(
      hostId,
      sessionId,
      parsed.data.version,
      parsed.data.section,
      parsed.data.transitionKey,
      parsed.data.sourceSeq,
    );
