...
/** Contract C4: live → paused. Version-guarded, host-owned. */
export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const parsed = startLiveSessionInputSchema.safeParse(await readBoundedJsonBody(request));
    if (!parsed.success) return apiError("일시정지 요청이 올바르지 않습니다.", "INVALID_REQUEST", 400);
    const session = await new LiveSessionService(getLiveSessionStore()).pause(
      hostId,
      sessionId,
      parsed.data.version,
    );
