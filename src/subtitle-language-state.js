import { normalizeSubtitleLanguageCode } from "./subtitle-languages.js";
...
function normalizeLanguageCode(value) {
  ...
  return normalizeSubtitleLanguageCode(normalized)
    || normalizeSubtitleLanguageCode(normalized.split("-")[0])
    || "unknown";
}
...
function analyzeTranscript(text) {
  ...
  if (hangul >= STRONG_HANGUL_CHARS && hangulRatio >= 0.2) {
    return { language: "ko", confidence: 0.96, isStrong: true, signal };
  }
  if (japanese >= STRONG_JAPANESE_CHARS && japanese / signal >= 0.35) {
    return { language: "ja", confidence: 0.94, isStrong: true, signal };
  }
...
export function createSubtitleLanguageState() {
  let lockedLanguage = "unknown";
  let lockedConfidence = 0;

  return {
    apply(providerLanguage = "") {
      return normalizeLanguageCode(providerLanguage);
    },
