...
function renderEmptyState(text, action = null) {
  const empty = document.createElement(action ? "section" : "p");
...
async function openCaptionSource() {
  ...
  const result = await bridge.showMainWindow();
  if (result?.ok === false) throw new Error(result.error || "MAIN_WINDOW_UNAVAILABLE");
  setStatus("NOVA 열림");
}
...
function renderAutomaticLane(lane, options = {}) {
  ...
  if (statusValue === "IDLE" || statusValue === "STALE" || isStale) {
    ...
    if (options.isWaitingForCaption) {
      automaticList.replaceChildren(renderEmptyState(
        "NOVA 메인 창에서 자막 또는 Live Call을 시작해야 상대방 질문을 기록할 수 있습니다.",
        { label: "NOVA에서 자막 시작", run: openCaptionSource },
      ));
    } else {
...
function applySnapshot(snapshot) {
  ...
  const isWaitingForCaption = Boolean(snapshot.coachSessionId)
    && snapshot.state === "LIVE"
    && !question
    && (snapshot.turns?.length ?? 0) === 0;
  ...
  renderAutomaticLane(snapshot.autoLane, { isWaitingForCaption });
  ...
  } else if (isWaitingForCaption) {
    setBadge(readyState, "자막 대기", "warn");
  } else {
