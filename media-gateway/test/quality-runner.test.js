...
test("quality runner is a no-network Gemini-only shared-engine check", async () => {
  ...
  assert.equal(metrics.voiceProvider, "gemini");
  ...
  assert.equal(metrics.liveSessions, 2);
  assert.equal(metrics.translatedAudioChunks, 1);
  assert.equal(metrics.sameLanguageAudioChunks, 0);
  assert.match(metrics.configFingerprint, /^gemini-caption-v1-[0-9a-f]{16}$/u);
});
